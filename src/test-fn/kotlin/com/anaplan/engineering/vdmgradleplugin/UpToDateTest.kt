...
class generateLaunchGenForAllTestUpToDateTest {
    ...
    fun dependencyUnpack_firstSucceedsSecondUpToDate_noDependencies() {
        ...
        runAndCheck(
            project, Task.dependencyUnpack, mapOf(
    ...
    fun dependencyUnpack_firstSucceedsSecondUpToDate_withDependency() {
        ...
        val project2 = createProject(
            moduleName = "B",
            config = createDependencyConfig(dependencies = listOf(Dependency(project1, "1.0.0")))
        )
        runAndCheck(
            project2, Task.dependencyUnpack, mapOf(
                Task.dependencyUnpack to TaskOutcome.SUCCESS
            )
        )
        runAndCheck(
    ...
    fun dependencyUnpack_OneOfManyDependenciesUpdated() {
        ...
        val project4 = createProject(
            moduleName = "D", config = createDependencyConfig(
                dependencies = listOf(
        ...
        runAndCheck(
            project4, Task.dependencyUnpack, mapOf(
                Task.dependencyUnpack to TaskOutcome.SUCCESS
            )
        )
        project4.buildGradle.writeText(
    ...
    fun dependencyUnpack_snapshotNeverUptodate() {
        ...
        val project2 = createProject(
            moduleName = "B",
            config = createDependencyConfig(dependencies = listOf(Dependency(project1, "1.0.0-SNAPSHOT")))
        )
        runAndCheck(
            project2, Task.dependencyUnpack, mapOf(
                Task.dependencyUnpack to TaskOutcome.SUCCESS
    ...
    fun typeCheckTests_firstSucceedsSecondUpToDate() {
        ...
        runAndCheck(
            project, Task.typeCheckTests, mapOf(
                Task.dependencyUnpack to TaskOutcome.SUCCESS,
    ...
    fun typeCheckTests_dependencyChangesButContentDoesnt() {
        ...
        runAndCheck(
            project2, Task.typeCheckTests, mapOf(
                Task.dependencyUnpack to TaskOutcome.SUCCESS,
                Task.typeCheckTests to TaskOutcome.SUCCESS
            )
        )
    ...
    private fun runAndCheck(
        project: Project,
        taskToRun: Task,
        expected: Map<Task, TaskOutcome>,
        expectBuildFailure: Boolean = false,
        failOnUnexpectedTask: Boolean = true
    ) {
        val result = executeBuild(
            projectDir = project.dir,
            clean = false,
            tasks = arrayOf(taskToRun.name),
            fail = expectBuildFailure
        )
